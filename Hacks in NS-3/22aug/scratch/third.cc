...
//    "/NodeList/" << wifiStaNodes.Get (1)->GetId () <<
//    "/$ns3::MobilityModel/CourseChange";

	  //  ASCII trace helper is used for trace tracking and capturing the packets by storing the details in a trace file.
	  AsciiTraceHelper ascii;
	  Ptr<OutputStreamWrapper> stream = ascii.CreateFileStream ("third_mobiliy.txt");
	  Config::Connect(oss.str (), MakeBoundCallback (&CourseChange, stream));
//	  oss.flush();


//	  Ptr<Object> theObject = wifiStaNodes.Get (1);
//	  Ptr<MobilityModel> mobilityModel = theObject->GetObject<MobilityModel> ();
//
//	  theObject->TraceConnect ("CourseChange", oss.str(), MakeCallback (&CourseChange));
