...
// Start of my code
      if (route->GetDestination()=="10.1.1.1")
    	  {
    	   NS_LOG_UNCOND (Simulator::Now ().GetSeconds ()<<"\t Gateway: "<<route->GetGateway()<<" Exist route to " << route->GetDestination () << " from source " << route->GetSource ());
    	  }
// end of the code
                             ...
                             UnicastForwardCallback ucb, ErrorCallback ecb)
{
  ...
  Ptr<OutputStreamWrapper> stream = ascii.CreateFileStream("first_routing.txt",std::ios::app);
...
//          NS_LOG_UNCOND(Simulator::Now ().GetSeconds ()<<"\t "<<route->GetSource ()<<" forwarding to " << dst << " from " << origin << " packet " << p->GetUid ());
          *stream->GetStream()<< Simulator::Now ().GetSeconds ()<<", Current node:"<<route->GetSource ()<<", Destination:" << dst << ", Source:" << origin << ", packet:" << p->GetUid ()<<std::endl;
