      ...
      if (InetSocketAddress::IsMatchingType (from))
        {
    	  ...
    	  NS_LOG_UNCOND ( Simulator::Now ().GetSeconds ()<< "s packet sink received "<<  packet->GetSize ()
					   << " bytes from "<< InetSocketAddress::ConvertFrom(from).GetIpv4 ()
//                       << " port " << InetSocketAddress::ConvertFrom (from).GetPort ()
//                       << " total Rx " << m_totalRx << " bytes "
                    		   <<"Packet ID:"<<packet->GetUid());
//							   <<" ack: "<<tcpheader.GetAckNumber()<<" Seq: "<<tcpheader.GetSequenceNumber());

// my edits
          AsciiTraceHelper ascii;
          Ptr<OutputStreamWrapper> stream_rx = ascii.CreateFileStream("first_routing.txt",std::ios::app);
          *stream_rx->GetStream()<< Simulator::Now ().GetSeconds ()<<",Route:0"
        		  	  <<", Node:" <<socket->GetNode()->GetId()<<",Destination:10.1.1.19"
...
//  }
//
//  positionAllocator->Add(Vector (1250.52,847.021, 0.0)); //0
//  positionAllocator->Add(Vector (1 * 200.0, 1*100.0, 0.0));
//  positionAllocator->Add(Vector (2 * 200.0, 2*100.0, 0.0));
//  positionAllocator->Add(Vector (852.229,1055.08, 0.0)); //3
//  positionAllocator->Add(Vector (1071.93,883.369, 0.0)); //4
//
//  for (int i = 5; i < nWifis-2; i++)
//    {
//        positionAllocator->Add(Vector (i * 200.0, i*100.0, 0.0));
//    }
//  positionAllocator->Add(Vector (187.232,1167.81, 0.0)); //18
//  positionAllocator->Add(Vector (409.256,967.52, 0.0)); //19
//
//
...
//  NS_LOG_UNCOND("any "<<Ipv4Address::GetAny());
  ...
  clientApp->Setup (clientTcpSocket, serverAddress, 536, nPackets, DataRate ("3Mbps"));
...
//  AsciiTraceHelper ascii;
  ...
  Ptr<OutputStreamWrapper> stream_packToSeq = ascii.CreateFileStream("packetidToSeq.txt");
  ...
  *stream_dat->GetStream()<<"\nDestn ID:"<<manet_DestnId<<"\nSource Id:"<<manet_sourceId<<"\nNPackets:"<<nPackets;
  ...
  aodv.PrintRoutingTableAllEvery(Seconds(1),stream_routing);
