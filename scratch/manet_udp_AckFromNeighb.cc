...
SendApp::SendDataPacket (SequenceNumber32 seq, uint32_t maxSize)
{
	...
	if ((seq <= m_highTxMark)&&(m_npacketsSent<m_nPackets))
...
//	Since we are not bothered about it, we keep it m_rto
	  if (isRetransmission)
		{ // This is a retransmit
		  ...
		  m_rto = Min (doubledRto, Time::FromDouble (10,  Time::S)); // upper thresholding by 60s
		}
...
//	      Simulator::ScheduleNow (&TcpSocketBase::NotifyDataSent, this, (seq + sz - m_highTxMark.Get ()));
	  ...
	  // Update highTxMark
	  m_highTxMark = std::min(std::max (seq + sz, m_highTxMark.Get ()).GetValue(),nPackets*PacketSize);
	  NS_LOG_UNCOND("m_hightxmark: "<<m_highTxMark);
	  return sz;
}
...
//	  m_nextTxSequence+=m_windowSize;
	  ...
	  m_highTxMark = std::min(std::max (m_nextTxSequence.Get() + m_windowSize, m_highTxMark.Get ()).GetValue(),
			  	  	  	  	  nPackets*PacketSize);
...
void
MyApp::AddPackettoBuffer(Ptr<Packet> packet)
{
	UdpAckHeader ackheader;
	packet->PeekHeader(ackheader);
	*(rxd_head+m_nPacketsinBuffer)=ackheader.GetSequenceNumber().GetValue();
	m_nPacketsinBuffer++;
	m_nPacketsTxd=0;
	NS_LOG_UNCOND("At node "<<GetNode()->GetId()<<" Number of packets in buffer: "<<m_nPacketsinBuffer);
}


void
MyApp::ForwardPackettoDst(Ptr<Socket> socket, Ptr<Packet> packet)
{
...
//    	  	  NS_LOG_UNCOND(Simulator::Now().GetSeconds()<<" My app, Node: "<< GetNode()->GetId()<<" Sending acknowledgment for "<<ackheader.GetSequenceNumber());
		  ...
		  ForwardPackettoDst(socket, packet);
      }
...
//	Setting up source application
    ...
    sendapp->SetStartTime (Seconds (15.));
